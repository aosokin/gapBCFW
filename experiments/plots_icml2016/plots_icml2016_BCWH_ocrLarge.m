...
lambdaValues       = {1e-4, 1e-3, 1e-1};
gapThresholdValues = {1e-9};
...
dataSet           = 'ocrLarge'; % don't forget to also change the result path and everything that depend on the dataset
pathSaveFig       = './figures';
...
xPassTargetVsGap     = 0:15:2000;
xLimGapVsPass        = [0 2000];
yLimGapVsPass        = [ 10^(-2) 1; 0.5*10^(-3), 0.1; 10^(-7), 10^(-3.5) ];

% second plot Gap vs Time
nPtsMarkersTimeVsGap = 3;
timeFormat           = 'h'; 
xTimeTargetVsGap     = [0:0.01:0.09, 0.1:0.1:6]; % this should have the same unit as time format
xLimGapVsTime        = [0, 6];
yLimGapVsTime        = yLimGapVsPass;

% third plot Test error vs Passes
nPtsMarkersTestErrorVsPass = 3;
xPassTargetVsTestError     = 1:10:2000;
xLimTestErrorVsPass        = [1, 2000];
yLimTestErrorVsPass        = [0.11, 0.16];
...
%    grid on
    ...
    if displayTrainError
        ...
        cur_pos = get(h,'Position');
        %set(h, 'Position', [cur_pos(1), cur_pos(2), 400, 260]);
        set(h, 'Position', [cur_pos(1), cur_pos(2), 300, 200]);

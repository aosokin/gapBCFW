...
%% path to the results
lambdaValues       = {1e-1, 100, 1000};
cachingValues      = {1, 0};
gapThresholdValues = {1e-8};
numPassesValues    = {20000};
timeBudgetValues   = {1*60};
...
dataSet           = 'horseSmall'; % don't forget to also change the result path and everything that depend on the dataset
pathSaveFig       = './figures';
saveFig           = false; % flag to save figure
iFig              = 1; % index of the first figure
alpha             = 0.0; % quantile parameter
...
xPassTargetVsGap     = [1, 10, 15, 20, 25, 30, 50:100:2050];
xLimGapVsPass        = [1, 2000];
yLimGapVsPass        = [ 3*10^(-2) 10^(-0.5); 10^(-7), 10^(-2); 10^(-7), 10^(-4)];

% second plot Gap vs Time
nPtsMarkersTimeVsGap = 3;
timeFormat           = 'h'; 
xTimeTargetVsGap     = [0.001:0.001:0.004,0.005:0.005:0.02, 0.03:0.03:0.55]; % this should have the same unit as time format
xLimGapVsTime        = [0, 0.5];
yLimGapVsTime        = yLimGapVsPass;

% third plot Test error vs Passes
nPtsMarkersTestErrorVsPass = 3;
xPassTargetVsTestError     = 1:500;
xLimTestErrorVsPass        = [1, 500];
yLimTestErrorVsPass        = [0.1, 0.2];

% Fourth plot (optional) Train error vs Passes
nPtsMarkersTrainErrorVsPass = 3;
xPassTargetVsTrainError     = 1:100:20000;
xLimTrainErrorVsPass        = [1 20000];

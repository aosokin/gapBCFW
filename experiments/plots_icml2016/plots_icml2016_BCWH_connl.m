...
%% CONLL
lambdaValues       = {0.0001 ,0.01, 0.1};
cachingValues      = {1, 0};
gapThresholdValues = {10^-8};
numPassesValues    = {3000};
timeBudgetValues   = {10*60}; 
...
dataSet           = 'conll'; % don't forget to also change the result path and everything that depend on the dataset
...
xPassTargetVsGap     = 1:0.5:70;
xLimGapVsPass        = [1 70];
yLimGapVsPass        = [ 2*10^(-3), 10^(-1); 10^(-4), 10^0; 10^(-5) 10^(-2)];

% second plot Gap vs Time
nPtsMarkersTimeVsGap = 3;
timeFormat           = 'h'; 
xTimeTargetVsGap     = (0.5:0.5:10); % this should have the same unit as time format
xLimGapVsTime        = [0.5, 10];
yLimGapVsTime        = [ 2*10^(-3), 10^(-1); 10^(-4), 10^0; 10^(-5), 10^(-2)];

% third plot Test error vs Passes
nPtsMarkersTestErrorVsPass = 3;
xPassTargetVsTestError     = 1:40;
xLimTestErrorVsPass        = [1, 40];
yLimTestErrorVsPass        = [0.1, 0.2];
...
for l=1:numel(lambdaValues)
    ...
    hLeg = plotMultipleMethodsWithErrorsBars(resultsGapVsOracle, numPassTarget, nPtsMarkers);
...
%    grid on
    ...
    hLeg = plotMultipleMethodsWithErrorsBars(resultsGapVsTime, numTimeTarget, nPtsMarkers);
    ...
    if displayTestError    
        ...
        hLeg = plotMultipleMethodsWithErrorsBars(resultsGapVsTestError, numPassTarget, nPtsMarkers);
    ...
    if displayTrainError
        ...
        hLeg = plotMultipleMethodsWithErrorsBars(resultsGapVsTrainError, numPassTarget, nPtsMarkers);

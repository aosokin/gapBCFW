...
gapThresholdValues = {1e-6};
numPassesValues    = {20000};
timeBudgetValues   = {6*60}; % 48h  
...
dataSet           = 'horseMedium'; % don't forget to also change the result path and everything that depend on the dataset
pathSaveFig       = './figures';
saveFig           = false; % flag to save figure
iFig              = 1;     % index of the first figure
alpha             = 0.00;  % quantile parameter (0.25 is good)
...
displayTestError  = false; % flag for test error
...
xPassTargetVsGap     = [1, 10:2:16, 20:15:200]; % target for linear regresion of passes
xLimGapVsPass        = [1 200]; % final plot limit (you can specify in an array format)
yLimGapVsPass        = [ 10^(-2) 10^(0); 10^(-5), 10^(-1); 10^(-5), 10^(-3)];
% limit for y axes, idem you can specify as it the case here the value
% differently for the different parameters

% second plot Gap vs Time
nPtsMarkersTimeVsGap = 3;
timeFormat           = 'h'; 
xTimeTargetVsGap     = [0.01, 0.05, 0.1, 0.15, 0.2:0.15:2.1]; % this should have the same unit as time format
xLimGapVsTime        = [0, 2];
yLimGapVsTime        = yLimGapVsPass;

% third plot Test error vs Passes
nPtsMarkersTestErrorVsPass = 3;
xPassTargetVsTestError     = 1:20:1500;
xLimTestErrorVsPass        = [1, 1600];

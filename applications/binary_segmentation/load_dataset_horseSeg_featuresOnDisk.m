function [patterns_train, labels_train, patterns_test, labels_test] = load_dataset_horseSeg_featuresOnDisk(data_name, data_path)
%lload_dataset_horseSeg_featuresOnDisk prepares the horseSeg data set in the BCFW format
%
% [patterns_train, labels_train, patterns_test, labels_test] = load_dataset_horseSeg_featuresOnDisk(data_name, data_path)
%
% Inputs:
...
fname = fullfile(data_path, [data_name, '_featuresOnDisk.mat']);
if (~exist(fname, 'file'))
    fprintf('Creating %s.mat for the first time...\n', data_name);
    [patterns_train, labels_train, patterns_test, labels_test] = convert_dataset_horseSeg_featuresOnDisk( data_name, data_path );
...
function [patterns_train, labels_train, patterns_test, labels_test] = convert_dataset_horseSeg_featuresOnDisk( data_name, data_path )
...
config.testFeatureFolder = fullfile('feature_files_test');
config.trainFeatureFolder = fullfile('feature_files_train', data_name);

if ~exist(fullfile(data_path, config.testFeatureFolder), 'dir')
    mkdir(fullfile(data_path, config.testFeatureFolder));
end
if ~exist(fullfile(data_path, config.trainFeatureFolder), 'dir')
    mkdir(fullfile(data_path, config.trainFeatureFolder));
end
tStart = tic;
...
for iIndex = startIndex : 1 : endIndex - 1
    ...
    edge_structure = h5read(curDataFile, ['/edge_structure_', indexStr] );
    features = single( h5read(curDataFile, ['/features_', indexStr] ) );
    ...
    patterns_test{num_images}.feature_file = fullfile( config.testFeatureFolder, [imageName, '.mat'] );
    ...
    % check the data
    if size(features, 2) ~= labels_test{num_images}.num_nodes
    ...
    features_pairwise = construct_pairwise_features( features, edge_structure+1 );
    
    % save features to file
    cur_file_name = fullfile( data_path, patterns_test{num_images}.feature_file );
    save(cur_file_name, 'features', 'features_pairwise', 'edge_structure', '-v6');
end
...
for iIndex = startIndex : 1 : endIndex - 1
    ...
    patterns_train{num_images}.feature_file = fullfile( config.trainFeatureFolder, [imageName, '.mat'] );
	...
	% check the data
    if size(features, 2) ~= labels_train{num_images}.num_nodes
    ...
    % save features to file
    cur_file_name = fullfile( data_path, patterns_train{num_images}.feature_file );
    save(cur_file_name, 'features', 'features_pairwise', 'edge_structure', '-v6');
end
